...
def who_has_this_operator():
    ...
    pattern_for_tele2 = r"^(?:\+7707|\+7747)"
    pattern_for_altel = r"^(?:\+7700|\+7708)"
    wassup = input("какого оператора хотите узнать? ").strip()
    try:
        with psycopg2.connect(**config) as conn:
            with conn.cursor() as cur:
                if wassup == 'билайн':
...
if __name__ == "__main__":
    ...
    check = input("хотите ли вы узнать у кого какой оператор связи? Ответьте Д, если согласны, Н - если нет").strip()
    ...
    print()
    print()
    permission = input("хотите узнать сколько операторов связи в базе данных? Ответьте Д, если согласны, Н - если нет ").strip()
    if permission == "Д":
        how_many_MB()
    else:
        print("Выход из системы")
