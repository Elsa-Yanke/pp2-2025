...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == FOOD_TIMEOUT:#чекаем нащ юзерский ивент
            food_x, food_y = spawn_food()
            superfood_x, superfood_y = spawn_food()
            pygame.time.set_timer(FOOD_TIMEOUT, 3000)  # обновляем таймер

    for i, j in snake: # рисуем змейку
    ...
    if move_t > move_i:
        ...
        x += dx * snake_size[0] #x = 300  300+ (1 * 30) = 330
        y += dy * snake_size[1] #y = 300  300+ (0 30 ) = 300
        snake.append((x, y))
    ...
    if (x < 0 or x > WIDTH - snake_size[0] or
        ...
        screen.blit(render_end, (WIDTH // 2 - 200, HEIGHT // 3))
        pygame.display.flip()
